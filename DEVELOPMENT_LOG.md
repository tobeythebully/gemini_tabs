...
### v1.2.2 (2026-01-08) - 전역 단축키 및 다중 창 지원
- **전역 단축키 (globalShortcut)**
  - webview에 포커스가 있어도 Cmd+1~9 탭 전환 작동
  - Cmd+T, Cmd+W도 전역으로 동작
- **다중 창 지원**
  - Cmd+N: 현재 창에서 새 탭 열기
  - Cmd+Shift+N: 새 창 열기
  - 각 창마다 독립적인 탭 관리

### v1.2.1 (2026-01-08) - 드롭다운 UX 개선
- **드롭다운 화면 적응**
  - 작은 화면(13인치 등)에서 드롭다운이 잘리지 않도록 뷰포트 경계 체크
  - 오른쪽/아래쪽 넘침 시 자동 위치 조정
- **스크롤 자동 따라가기**
  - 화살표 키로 항목 이동 시 선택 항목이 보이도록 자동 스크롤
- **Enter 키 지원**
  - Tab 뿐 아니라 Enter 키로도 프롬프트 선택 가능

### v1.2.0 (2026-01-07) - 슬래시 드롭다운
- **슬래시 드롭다운 기능 추가**
  - `/` 입력 시 명령어 목록 드롭다운 표시
  - ⬆️⬇️ 방향키로 이동, Tab/클릭으로 선택
  - Esc로 닫기
  - 입력하면서 필터링 (부분 일치)
- **TrustedHTML 정책 호환**
  - innerHTML 대신 DOM createElement 사용
- **빌드 가이드 추가**
  - `docs/BUILD_GUIDE.md` 생성
  - dist 폴더 버전별 정리 가이드
